...
export const useGame = () => {
  ...
  const cartasIniciais = Array.from({ length: totalPokemon }, (_, i) => {
    ...
    return {
      // me poupando de criar um array quilométrico para trazer os pokémon.
      id_pokemon: id,
  ...
  useEffect(() => {
    //	Mantém o jogo atualizado com as configs atuais.
  ...
  const prepFase = (resetTotal = false) => {
    // Prepara a fase.
    if (resetTotal) {
      setFaseAtual(1);
      return;
    ...
    const shuffled = [...cartasSorteadas, ...cartasSorteadas] // Gera os pares e os prepara para a fase:
      .sort(() => Math.random() - 0.5) // Embaralha;
      .map((carta) => {
        return { ...carta, id: Math.random(), combinado: false };
      }); // Seta todas as cartas como 'não combinadas' (combinado: false).
  ...
  useEffect(() => {
    if (choice1 && choice2) {
      // Se as duas escolhas já foram feitas,
      ...
      if (choice1.src === choice2.src) {
        ...
        setCartas((escolhidas) => {
          // pega as cartas escolhidas e
          return escolhidas.map((carta) => {
            // mapeia cada carta:
            if (carta.src === choice1.src) {
              // Mais uma comparação para o .map(), e se confirmar,
        ...
        // Se as cartas não forem iguais (erro):
